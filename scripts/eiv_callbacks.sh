...
#driver function to install gpu drivers and related packages on ADLN-platforms
gpu_drivers_adln_install(){
    if [ $RELEASE == "22.04" ]; then
        if [ $dGPU -ge 2 ]; then
            kern_upgrade_dgpu
        elif dpkg --compare-versions "$(uname -r | cut -d'-' -f1)" eq 6.1.38 ; then
            install_igpu_drivers
        else
            install_igpu_drivers        
        fi
